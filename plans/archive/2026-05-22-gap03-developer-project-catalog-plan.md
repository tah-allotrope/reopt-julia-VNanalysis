...
> **Implementation status (2026-05-29): COMPLETE.** All three phases implemented with Red/Green TDD.
> 30 tests pass across `test_project_catalog.py` (10), `test_matching.py` (11), `test_matching_e2e.py` (9);
> 205 passed across the cleanly-collecting Python suite. Phase reports under
> `reports/2026-05-29-gap03-phase-0{1,2,3}.html`; match artifacts under `artifacts/reports/match_*.json`.
...
**Tasks**
- [x] TASK-01-01: Create `data/projects/catalog_schema.json` defining the project record schema with fields: `project_id`, `name`, `developer`, `location` (lat, lon, province, region), `technology` (solar/wind/solar_bess/wind_bess/hybrid), `capacity_mw`, `bess_mw`, `bess_mwh`, `grid_connection` (onsite_private_wire/grid_connected_22kv/grid_connected_110kv), `indicative_strike_usc_kwh`, `available_from`, `dppa_structure` (private_wire/virtual_cfd/physical_dppa), `status` (operational/construction/development/prospective), `generation_profile_path` (optional path to 8760 JSON), `notes`.
- [x] TASK-01-02: Create `data/projects/saigon18_onsite_solar_bess.json` — 40.36 MWp PV + 20 MW/66 MWh BESS, onsite private-wire, south region, derived from saigon18 case study.
- [x] TASK-01-03: Create `data/projects/ninhsim_offsite_solar_wind.json` — 14 MW PV + 40 MW wind, grid-connected virtual CfD, south region, derived from ninhsim optimized result.
- [x] TASK-01-04: Create `data/projects/north_thuan_offsite_solar_wind_bess.json` — 30 MW solar + 20 MW wind + 10 MW/40 MWh BESS, grid-connected, central region, derived from north_thuan.
- [x] TASK-01-05: Create `data/projects/real_project_onsite_solar_bess.json` — 3.2 MWp PV + 1 MW/2.2 MWh BESS, onsite 22kV, south region, derived from AGENTS.md real-project basis.
- [x] TASK-01-06: Create `data/projects/prospective_offsite_wind.json` — 50 MW wind, grid-connected 110kV, central region, prospective status, to demonstrate matching against a large-scale project.
- [x] TASK-01-07: Add `tests/python/integration/test_project_catalog.py` with schema validation tests for all 5 seed projects.
...
**Exit Criteria**
- [x] All 5 seed projects pass schema validation.
- [x] Each project has a valid `project_id`, `technology`, `capacity_mw`, `grid_connection`, and `dppa_structure`.
...
**Tasks**
- [x] TASK-02-01: Create `src/python/reopt_pysam_vn/integration/matching.py` with `match_projects_to_factory(factory_load_result, project_catalog, tariff_params) -> list[ProjectMatch]`.
- [x] TASK-02-02: Implement `ProjectMatch` dataclass: `project_id`, `project_name`, `overall_score` (0-100), `dimension_scores` (dict of 5 dimensions), `fit_explanation` (human-readable string), `flags` (list of warnings/blockers).
- [x] TASK-02-03: Implement physical fit scoring (0-100): if project has a generation profile, compute hourly load-vs-generation overlap using the same logic as `rank_case_study_offtakers.py` (self-consumption ratio, curtailment ratio, BESS absorption headroom). If no generation profile, estimate from capacity vs peak demand ratio.
- [x] TASK-02-04: Implement geographic fit scoring (0-100): same-region = 100, adjacent-region = 70, cross-country = 40. For onsite projects, require same location (score 0 if not co-located). For offsite, deduct based on distance proxy.
- [x] TASK-02-05: Implement capacity fit scoring (0-100): project annual generation vs factory annual consumption ratio. Score peaks at 0.3-0.7 ratio (sweet spot for DPPA), degrades for undersized (< 0.1) or oversized (> 1.5).
- [x] TASK-02-06: Implement commercial fit scoring (0-100): compare project indicative strike vs factory EVN baseline cost. Positive buyer savings = high score, buyer premium = low score, missing strike data = neutral (50).
- [x] TASK-02-07: Implement regulatory fit scoring (0-100): check Decree 57 export-cap headroom for onsite, grid-connection voltage compatibility, DPPA structure eligibility. Flag blockers (e.g., private-wire project can't serve an offsite factory).
- [x] TASK-02-08: Add `tests/python/integration/test_matching.py` with tests for: saigon18 factory matched against all 5 projects (expect onsite solar+BESS to rank highest), ninhsim factory matched against all 5 projects, edge case where no project is viable.
...
**Exit Criteria**
- [x] `match_projects_to_factory()` returns a ranked list for saigon18 factory with the onsite project scoring highest in physical fit.
- [x] Each `ProjectMatch` has non-zero scores in all 5 dimensions.
- [x] Regulatory fit correctly flags private-wire projects as incompatible with offsite factories.
...
**Tasks**
- [x] TASK-03-01: Create `scripts/python/integration/match_factory_to_projects.py` CLI accepting `--factory <path>` (ingested load artifact or raw file), `--catalog <dir>` (default: `data/projects/`), `--output <path>`, `--top-n <int>` (default: 5).
- [x] TASK-03-02: Implement match result JSON artifact with: `factory_summary`, `matches` (ranked list of `ProjectMatch`), `catalog_size`, `match_timestamp`, `scoring_weights`.
- [x] TASK-03-03: Run matching for all 6 existing case-study factories against the 5-project catalog and verify results are directionally reasonable.
- [x] TASK-03-04: Add `tests/python/integration/test_matching_e2e.py` validating the full CLI pipeline.
- [x] TASK-03-05: Create convenience wrapper at `scripts/python/match_factory_to_projects.py`.
...
**Exit Criteria**
- [x] `python scripts/python/match_factory_to_projects.py --factory scenarios/case_studies/ninhsim/NinhsimSample.csv --output /tmp/matches.json` produces a valid ranked match artifact.
- [x] All matching tests pass: `python -m pytest tests/python/integration/test_project_catalog.py tests/python/integration/test_matching.py tests/python/integration/test_matching_e2e.py -q`.
