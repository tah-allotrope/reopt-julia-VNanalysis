...
### Presentation — Allotrope DPPA insights deck ✅ (2026-06-04, fresh Linux remote session)
- Verified STEP 1 fully complete on a FRESH Linux checkout: all 4 phases committed/pushed (PHASE-04 = 125dad8). Re-ran the suite on Linux with real PySAM 7.1 (wheel IS available on Linux, installed into `.venv` from requirements.txt): **25 passed** (20 Samsung phase tests + 5 regime-multi). PySAM-driven PVWatts path exercised (not just synthetic fallback).
- Built `present/Allotrope DPPA insights.pptx` (12 slides) via new `scripts/python/integration/generate_samsung_ttc_deck.py` (python-pptx). Deck commit 383b403, pushed to origin/main.
- **Template provenance / blocker noted:** the named template `present/Allotrope DPPA insights.pptx` was NOT present in this checkout (empty `present/` folder, not in git history). Used the canonical Allotrope conformance template `reports/decks/conformance/template/allotrope-template.pptx` as the base — the same master/layout/logo set the production `reports/decks/2026-04-21-dppa-case-3-allotrope.pptx` is built from. Preserved masters, layouts, logos (slide-1 banner + top-right logos; slide-12 background + logo), fonts, colors; only swapped text/table/chart content at existing shape positions. No original to back up (folder was empty).
- Slide map: (1) title; (2) deal-at-a-glance table; (3) deal info + strike basis table; (4) 5 finding cards; (5) plant/solar table + native chart; (6) strike-sweep buyer-net + developer-NPV charts; (7) 4-phase methodology; (8) DPPA-adder sensitivity chart; (9) regime-stress chart; (10) Decree 57/2025 market context; (11) risks/directional caveats; (12) closing + sources. Every deal number labeled DIRECTIONAL (undisclosed strike + proxy CFMP basis). Data from `artifacts/reports/samsung_ttc/*.json` + `research/2026-06-04_samsung-ttc-dppa.md`.
- Skills `/present` and `/report` are NOT available in this remote session → built the deck directly with python-pptx (mirrors `scripts/python/generate_deck_3.py`). LibreOffice is installed but cannot load pptx in this container (fails on the original template too — env limitation, not the file); validated structure by reloading with python-pptx instead.
- ENV CORRECTION to earlier note: on Linux the `nrel-pysam>=7.1` wheel installs fine; use POSIX `. .venv/bin/activate` + `python` (the `.venv\Scripts\python.exe` path is Windows-only).
