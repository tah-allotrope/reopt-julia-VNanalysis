# Archive: July 2026 Case Studies deck verification (calibrate-then-validate)

> Rotated out of `activeContext.md` on 2026-07-04 — work is complete and committed.
> See git log `feat(ceba-deck): PHASE-01..PHASE-05` for the commits.
...
- 13 ok (≤ ±1%) — A-bucket (10) + worked example B01-B04 (4) + A11 (1, offset by A17 moving to warn)
- 4 warn (1–5% / structural reconcile) — A02 ratio, A07 Kpp collapse, A12 FMP cite, A17 analysis years
- 15 info (qualitative / method-level / directional) — A01, A15, A16, B05, C01-C10
...
- **Actual outcome (per the report): 16 ok / 5 warn (A02, A07, A12, B05, B06) / 14 info / 0 bad.**
  (Earlier drafts of this file said "5 bad" — that was never the committed result; DEC-007 means
  PySAM null-IRR is filed info, never forced to bad.)
- Headline: Cases 5/6 (deck IRR 16.9% / 26.9%) not reproducible under proxy CAPEX — repo gives
  *negative* DSCR at strike 2,000. A12: deck FMP 1,426.6 vs repo center 1,700. This the July deck
...
**Final verdict counts (50 checks):** 13 ok / 4 warn / 15 info / 4 bad / 0 skip / 0 err / 14 calibrated
- 4 bad (B21-B24) are the four slide-25 gate rows — the deck's specific PASS/FAIL
  numbers don't reproduce under disclosed terms (the qualitative "0 of 56"
  headline does reproduce).
- 14 calibrated are the Case 5/6 family — the seller IRR hits the deck value +/-0.2pp at
